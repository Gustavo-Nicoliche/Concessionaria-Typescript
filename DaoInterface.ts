// Declaração de uma interface genérica chamada DaoInterface
export interface DaoInterface<T> { 
    // Propriedade que representa o nome da tabela no banco de dados
    nomeTabela: string;

    // Método genérico para inserir um objeto do tipo T
    inserir(object: T): boolean;

    // Método genérico para atualizar um objeto do tipo T
    atualizar(object: T): boolean;

    // Método genérico para remover um objeto baseado em um ID e retornar o objeto removido
    remover(id: number): T;

    // Método genérico para selecionar um objeto baseado em um ID e retornar o objeto encontrado
    selecionar(id: number): T;

    // Método genérico para selecionar todos os objetos e retornar um array do tipo T
    selecionarTodos(): [T];
}
